...
typedef struct {	/* segy - trace identification header */
	...
	int corssline; /* corssline number
			   byte# 193-196
			*/
	int shot;/* incident angle the units are degree not radian.
			   byte# 197-200
			*/
	int incident; /* number of traces
			    byte# 201-204
			*/
	int ffile; /* file NO.
			    byte# 205-208
...
extern "C"
{
	...
	int segyread (const char *ifile ,           segy *tr_h, float *seis_data, int cTrace);
	int segywrite(const char *ofile, bhed re_h, segy  tr_h, float *seis_data, int cTrace);
	...
	int segywriteIBM(const char *ofile, bhed re_h, segy  tr_h, float *seis_data, int cTrace);
	int write2asc(const char *ofile, bhed re_h, segy  tr_h, float *seis_data, int cTrace);
	...
	int pickShotgather(const char *file, segy *trBuf,  float *traceBuf, long int *size, int *ntr, int *flag );

	int getSgyInfoF    (bhed *re,   int   *ntrace,   int *ns, int *dt);
	int segyreadF      (segy *tr_h, float *trace,    int cTrace);
	int segywriteF(bhed re_h, segy  tr_h, float *seis_data, int cTrace);
	int segywriteFIBM(bhed re_h, segy  tr_h, float *seis_data, int cTrace);
	...
	int GetTrace (FILE *fp, Segy *tr);
	int SaveTrace(FILE *fp, Segy  tr);
	int GetTr(Segy *tr);
	int PutTr(Segy  tr);
